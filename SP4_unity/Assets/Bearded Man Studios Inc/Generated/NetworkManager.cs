...
namespace BeardedManStudios.Forge.Networking.Unity
{
	public partial class NetworkManager : MonoBehaviour
	{
		...
		public GameObject[] FlamethrowerNetworkObject = null;
		public GameObject[] GameLogicNetworkObject = null;
		public GameObject[] HealthPotionNetworkObject = null;
		...
		private void CaptureObjects(NetworkObject obj)
		{
			...
			else if (obj is EnemyNetworkObject)
			{
				MainThreadManager.Run(() =>
				{
					NetworkBehavior newObj = null;
					if (!NetworkBehavior.skipAttachIds.TryGetValue(obj.NetworkId, out newObj))
					{
						if (EnemyNetworkObject.Length > 0 && EnemyNetworkObject[obj.CreateCode] != null)
						{
							var go = Instantiate(EnemyNetworkObject[obj.CreateCode]);
							newObj = go.GetComponent<EnemyBehavior>();
						}
					}
				...
				MainThreadManager.Run(() =>
				{
					...
					if (!NetworkBehavior.skipAttachIds.TryGetValue(obj.NetworkId, out newObj))
					{
						if (EnemySpawnerNetworkObject.Length > 0 && EnemySpawnerNetworkObject[obj.CreateCode] != null)
						{
							...
							newObj = go.GetComponent<EnemySpawnerBehavior>();
						}
					}
			...
			else if (obj is FlamethrowerNetworkObject)
			{
				MainThreadManager.Run(() =>
				{
					NetworkBehavior newObj = null;
					if (!NetworkBehavior.skipAttachIds.TryGetValue(obj.NetworkId, out newObj))
					{
						if (FlamethrowerNetworkObject.Length > 0 && FlamethrowerNetworkObject[obj.CreateCode] != null)
						{
							var go = Instantiate(FlamethrowerNetworkObject[obj.CreateCode]);
							newObj = go.GetComponent<FlamethrowerBehavior>();
						}
					}
			...
			else if (obj is HealthPotionNetworkObject)
			{
				MainThreadManager.Run(() =>
				{
					NetworkBehavior newObj = null;
					if (!NetworkBehavior.skipAttachIds.TryGetValue(obj.NetworkId, out newObj))
					{
						if (HealthPotionNetworkObject.Length > 0 && HealthPotionNetworkObject[obj.CreateCode] != null)
						{
							var go = Instantiate(HealthPotionNetworkObject[obj.CreateCode]);
							newObj = go.GetComponent<HealthPotionBehavior>();
						}
					}
			...
			else if (obj is NetworkMapGenerationNetworkObject)
			{
				MainThreadManager.Run(() =>
				{
					NetworkBehavior newObj = null;
					if (!NetworkBehavior.skipAttachIds.TryGetValue(obj.NetworkId, out newObj))
					{
						if (NetworkMapGenerationNetworkObject.Length > 0 && NetworkMapGenerationNetworkObject[obj.CreateCode] != null)
						{
							var go = Instantiate(NetworkMapGenerationNetworkObject[obj.CreateCode]);
							newObj = go.GetComponent<NetworkMapGenerationBehavior>();
						}
					}
			...
			else if (obj is PlayerVehicleNetworkObject)
			{
				MainThreadManager.Run(() =>
				{
					NetworkBehavior newObj = null;
					if (!NetworkBehavior.skipAttachIds.TryGetValue(obj.NetworkId, out newObj))
					{
						if (PlayerVehicleNetworkObject.Length > 0 && PlayerVehicleNetworkObject[obj.CreateCode] != null)
						{
		...
		[Obsolete("Use InstantiateEnemy instead, its shorter and easier to type out ;)")]
		public EnemyBehavior InstantiateEnemyNetworkObject(int index = 0, Vector3? position = null, Quaternion? rotation = null, bool sendTransform = true)
		{
			var go = Instantiate(EnemyNetworkObject[index]);
			var netBehavior = go.GetComponent<EnemyBehavior>();
			var obj = netBehavior.CreateNetworkObject(Networker, index);
			go.GetComponent<EnemyBehavior>().networkObject = (EnemyNetworkObject)obj;
		...
		[Obsolete("Use InstantiateFlamethrower instead, its shorter and easier to type out ;)")]
		public FlamethrowerBehavior InstantiateFlamethrowerNetworkObject(int index = 0, Vector3? position = null, Quaternion? rotation = null, bool sendTransform = true)
		{
		...
		[Obsolete("Use InstantiateHealthPotion instead, its shorter and easier to type out ;)")]
		public HealthPotionBehavior InstantiateHealthPotionNetworkObject(int index = 0, Vector3? position = null, Quaternion? rotation = null, bool sendTransform = true)
		{
			var go = Instantiate(HealthPotionNetworkObject[index]);
			var netBehavior = go.GetComponent<HealthPotionBehavior>();
			var obj = netBehavior.CreateNetworkObject(Networker, index);
			go.GetComponent<HealthPotionBehavior>().networkObject = (HealthPotionNetworkObject)obj;
		...
		[Obsolete("Use InstantiateNetworkMapGeneration instead, its shorter and easier to type out ;)")]
		public NetworkMapGenerationBehavior InstantiateNetworkMapGenerationNetworkObject(int index = 0, Vector3? position = null, Quaternion? rotation = null, bool sendTransform = true)
		{
			var go = Instantiate(NetworkMapGenerationNetworkObject[index]);
			var netBehavior = go.GetComponent<NetworkMapGenerationBehavior>();
			var obj = netBehavior.CreateNetworkObject(Networker, index);
			go.GetComponent<NetworkMapGenerationBehavior>().networkObject = (NetworkMapGenerationNetworkObject)obj;
		...
		[Obsolete("Use InstantiatePlayerVehicle instead, its shorter and easier to type out ;)")]
		public PlayerVehicleBehavior InstantiatePlayerVehicleNetworkObject(int index = 0, Vector3? position = null, Quaternion? rotation = null, bool sendTransform = true)
		{
			var go = Instantiate(PlayerVehicleNetworkObject[index]);
		...
		/// <summary>
		/// Instantiate an instance of Enemy
		/// </summary>
		/// <returns>
		/// A local instance of EnemyBehavior
		/// </returns>
		/// <param name="index">The index of the Enemy prefab in the NetworkManager to Instantiate</param>
		/// <param name="position">Optional parameter which defines the position of the created GameObject</param>
		/// <param name="rotation">Optional parameter which defines the rotation of the created GameObject</param>
		/// <param name="sendTransform">Optional Parameter to send transform data to other connected clients on Instantiation</param>
		public EnemyBehavior InstantiateEnemy(int index = 0, Vector3? position = null, Quaternion? rotation = null, bool sendTransform = true)
		{
			var go = Instantiate(EnemyNetworkObject[index]);
			var netBehavior = go.GetComponent<EnemyBehavior>();
		...
		/// <summary>
		/// Instantiate an instance of EnemySpawner
		/// </summary>
		/// <returns>
		/// A local instance of EnemySpawnerBehavior
		/// </returns>
		/// <param name="index">The index of the EnemySpawner prefab in the NetworkManager to Instantiate</param>
		/// <param name="position">Optional parameter which defines the position of the created GameObject</param>
		/// <param name="rotation">Optional parameter which defines the rotation of the created GameObject</param>
		/// <param name="sendTransform">Optional Parameter to send transform data to other connected clients on Instantiation</param>
		...
		/// </summary>
		/// <returns>
		/// A local instance of FlamethrowerBehavior
		/// </returns>
		/// <param name="index">The index of the Flamethrower prefab in the NetworkManager to Instantiate</param>
		/// <param name="position">Optional parameter which defines the position of the created GameObject</param>
		/// <param name="rotation">Optional parameter which defines the rotation of the created GameObject</param>
		/// <param name="sendTransform">Optional Parameter to send transform data to other connected clients on Instantiation</param>
		public FlamethrowerBehavior InstantiateFlamethrower(int index = 0, Vector3? position = null, Quaternion? rotation = null, bool sendTransform = true)
		{
			var go = Instantiate(FlamethrowerNetworkObject[index]);
			var netBehavior = go.GetComponent<FlamethrowerBehavior>();
		...
		/// <summary>
		/// Instantiate an instance of HealthPotion
		/// </summary>
		/// <returns>
		/// A local instance of HealthPotionBehavior
		/// </returns>
		/// <param name="index">The index of the HealthPotion prefab in the NetworkManager to Instantiate</param>
		/// <param name="position">Optional parameter which defines the position of the created GameObject</param>
		/// <param name="rotation">Optional parameter which defines the rotation of the created GameObject</param>
		/// <param name="sendTransform">Optional Parameter to send transform data to other connected clients on Instantiation</param>
		public HealthPotionBehavior InstantiateHealthPotion(int index = 0, Vector3? position = null, Quaternion? rotation = null, bool sendTransform = true)
		{
			var go = Instantiate(HealthPotionNetworkObject[index]);
			var netBehavior = go.GetComponent<HealthPotionBehavior>();
		...
		/// <summary>
		/// Instantiate an instance of NetworkMapGeneration
		/// </summary>
		/// <returns>
		/// A local instance of NetworkMapGenerationBehavior
		/// </returns>
		/// <param name="index">The index of the NetworkMapGeneration prefab in the NetworkManager to Instantiate</param>
		/// <param name="position">Optional parameter which defines the position of the created GameObject</param>
		/// <param name="rotation">Optional parameter which defines the rotation of the created GameObject</param>
		/// <param name="sendTransform">Optional Parameter to send transform data to other connected clients on Instantiation</param>
		public NetworkMapGenerationBehavior InstantiateNetworkMapGeneration(int index = 0, Vector3? position = null, Quaternion? rotation = null, bool sendTransform = true)
		{
			var go = Instantiate(NetworkMapGenerationNetworkObject[index]);
			var netBehavior = go.GetComponent<NetworkMapGenerationBehavior>();
		...
		/// <summary>
		/// Instantiate an instance of PlayerVehicle
		/// </summary>
		/// <returns>
		/// A local instance of PlayerVehicleBehavior
		/// </returns>
		/// <param name="index">The index of the PlayerVehicle prefab in the NetworkManager to Instantiate</param>
		/// <param name="position">Optional parameter which defines the position of the created GameObject</param>
		/// <param name="rotation">Optional parameter which defines the rotation of the created GameObject</param>
		/// <param name="sendTransform">Optional Parameter to send transform data to other connected clients on Instantiation</param>
		public PlayerVehicleBehavior InstantiatePlayerVehicle(int index = 0, Vector3? position = null, Quaternion? rotation = null, bool sendTransform = true)
		{
			var go = Instantiate(PlayerVehicleNetworkObject[index]);
			var netBehavior = go.GetComponent<PlayerVehicleBehavior>();
